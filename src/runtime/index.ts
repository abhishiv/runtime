...
import {
  ...
  getDependencyParentLogicalTree,
  isNativeModule,
  getNativeModule,
...
class Runtime implements IRuntime {
  ...
  async importModule(path: string, lTree?: ILogicalTree | null): Promise<unknown> {
    ...
    const logicalTreeP = lTree || (await getLogicalTree(this.props.fs, this.props.workDir))
    const moduleDependency = await convertPathToModuleDependency(this, path, path, logicalTreeP, undefined)

    if (!moduleDependency) {
      console.error('importModule', moduleDependency, path, logicalTreeP)
      return null
    }
    ...
    const loadedLoad = await (async () => {
      if (moduleDependency.type === 'source' && moduleDependency.pkg.address === '') {
    ...
    const promises = dependencies.map((dependencyModule) => {
      return async () => {
        ...
        const logicalTree = dependencyModule.pkg
        if (isNativeModule(parsed.name)) {
          return
        }

        const tree = logicalTree || (await getDependencyParentLogicalTree(this, logicalTree, parsed.name))
        const depModule = await this.importModule(dependencyModule.resolvedFSPath, logicalTree)
        return depModule
      }
    })
    ...
    const evaledLoad = await evalModule(processedLoad, this.props.evalFunction, this)
