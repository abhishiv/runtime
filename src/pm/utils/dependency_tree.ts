...
export async function getLogicalTree(
  fs: IFilesystem,
  packageRoot: string
): Promise<LogicalTree> {
  ...
  const PackageLockText = (
    (await promisify(readFile)(
      nodePath.join(packageRoot, "package-lock.json")
    )) as unknown as Buffer
  ).toString();
...
export function getAddressList(logicalTree: ILogicalTree): {
  [addressName: string]: ILogicalTree;
} {
  ...
  crawl(
    ...
    },
    {
      getChildren: (node) => {
        ...
        const list = Array.from(node.dependencies.values()).filter(
          (el) => !seenMap[el.address]
        );
        list.forEach((pkg) => {
...
export function logicalTreeAdressToFSPath(address: string) {
  const localPath = (address.split(":") as string[]).reduce(function (
    state,
    item,
    index
  ) {
